"""
MiniMax API Client - Implementation for interacting with MiniMax API
"""

import logging
import time
import requests
from typing import Dict, List, Optional, Any
import os

logger = logging.getLogger(__name__)
from dataclasses import dataclass

from core.config import LLM_CONFIG, API_KEY_ENV_VARS


@dataclass
class MiniMaxMessage:
    """Represents a message in the conversation"""
    role: str  # "system", "user", or "assistant"
    content: Any  # str or list of content parts (for vision)


class MiniMaxAPIClient:
    """Client for interacting with MiniMax API"""

    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None):
        """
        Initialize MiniMax API client

        Args:
            api_key: Your MiniMax API key (can also be set via MINIMAX_API_KEY env var)
            base_url: Base URL for MiniMax API (optional, uses config value if not provided)
        """
        self.api_key = api_key or os.getenv(API_KEY_ENV_VARS["minimax"])
        self.base_url = base_url or LLM_CONFIG["minimax"]["base_url"]

        if not self.api_key:
            raise ValueError(f"API key is required. Set {API_KEY_ENV_VARS['minimax']} environment variable or pass api_key parameter.")

    def _make_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Make HTTP request to MiniMax API"""
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
        }

        max_attempts = 3
        for attempt in range(1, max_attempts + 1):
            try:
                response = requests.post(self.base_url, headers=headers, json=payload, timeout=240)
                response.raise_for_status()
                return response.json()
            except requests.exceptions.Timeout as e:
                if attempt < max_attempts:
                    logger.warning(f"API request timed out (attempt {attempt}/{max_attempts}), retrying...")
                    continue
                raise Exception(f"API request failed: {e}")
            except requests.exceptions.HTTPError as e:
                if response.status_code == 429 and attempt < max_attempts:
                    wait = 5 * attempt
                    logger.warning(f"Rate limited (429), waiting {wait}s before retry {attempt}/{max_attempts}...")
                    time.sleep(wait)
                    continue
                if response.status_code >= 500 and attempt < max_attempts:
                    wait = 3 * attempt
                    logger.warning(f"Server error ({response.status_code}), waiting {wait}s before retry {attempt}/{max_attempts}...")
                    time.sleep(wait)
                    continue
                raise Exception(f"API request failed: {e}")
            except requests.exceptions.RequestException as e:
                raise Exception(f"API request failed: {e}")

    def chat_completion(
        self,
        messages: List[MiniMaxMessage],
        model: Optional[str] = None,
        max_tokens: Optional[int] = None,
        temperature: Optional[float] = None,
        top_p: Optional[float] = None,
        stream: Optional[bool] = None
    ) -> Dict[str, Any]:
        """
        Generate chat completion using MiniMax API

        Args:
            messages: List of conversation messages
            model: Model to use (e.g., MiniMax-M2.7, MiniMax-M2.5)
            max_tokens: Maximum tokens to generate
            temperature: Sampling temperature (0.0 to 1.0)
            top_p: Top-p sampling parameter
            stream: Whether to stream the response

        Returns:
            API response dictionary
        """
        model = model or LLM_CONFIG["minimax"]["default_model"]
        max_tokens = max_tokens or LLM_CONFIG["minimax"]["default_params"]["max_tokens"]
        temperature = temperature if temperature is not None else LLM_CONFIG["minimax"]["default_params"]["temperature"]
        top_p = top_p or LLM_CONFIG["minimax"]["default_params"]["top_p"]
        stream = stream if stream is not None else LLM_CONFIG["minimax"]["default_params"]["stream"]

        payload = {
            "model": model,
            "messages": [{"role": msg.role, "content": msg.content} for msg in messages],
            "max_tokens": max_tokens,
            "temperature": temperature,
            "top_p": top_p,
            "stream": stream,
            # Separate reasoning from content so the returned content field
            # contains only the final answer (critical for downstream JSON parsing).
            "reasoning_split": True
        }

        return self._make_request(payload)

    def simple_chat(
        self,
        prompt: str,
        model: Optional[str] = None,
        temperature: Optional[float] = None,
    ) -> str:
        """
        Simple chat interface - send a prompt and get response

        Args:
            prompt: User prompt/question
            model: Model to use (optional, uses config value if not provided)

        Returns:
            Generated response text
        """
        model = model or LLM_CONFIG["minimax"]["default_model"]

        messages = [MiniMaxMessage(role="user", content=prompt)]
        response = self.chat_completion(messages, model=model, temperature=temperature)

        try:
            message = response["choices"][0]["message"]
            content = message.get("content")
        except (KeyError, IndexError):
            raise Exception(f"Unexpected response format: {response}")
        # MiniMax reasoning models may return empty content when
        # max_tokens is exhausted by reasoning. Treat as a truncation error.
        if not content:
            reasoning_details = message.get("reasoning_details", [])
            if reasoning_details:
                reasoning_text = reasoning_details[0].get("text", "") if reasoning_details else ""
                raise Exception(
                    f"Model exhausted max_tokens on reasoning without producing content. "
                    f"Consider increasing max_tokens. Partial reasoning: {reasoning_text[:200]}..."
                )
            raise Exception(f"Model returned null content (possible content filter or empty response). Response: {response}")
        return content

    def vision_chat(self, prompt: str, images: List[str], model: Optional[str] = None) -> str:
        """Send text + base64 images to vision-capable model."""
        model = model or LLM_CONFIG["minimax"]["default_model"]
        content = [{"type": "text", "text": prompt}]
        for img_b64 in images:
            content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{img_b64}"}})
        messages = [MiniMaxMessage(role="user", content=content)]
        response = self.chat_completion(messages, model=model)
        return response["choices"][0]["message"].get("content") or ""

    def conversation_chat(
        self,
        messages: List[MiniMaxMessage],
        system_prompt: Optional[str] = None,
        model: Optional[str] = None
    ) -> str:
        """
        Multi-turn conversation chat

        Args:
            messages: List of conversation messages
            system_prompt: Optional system prompt to set context
            model: Model to use (optional, uses config value if not provided)

        Returns:
            Generated response text
        """
        model = model or LLM_CONFIG["minimax"]["default_model"]

        conversation = []

        if system_prompt:
            conversation.append(MiniMaxMessage(role="system", content=system_prompt))

        conversation.extend(messages)

        response = self.chat_completion(conversation, model=model)

        try:
            message = response["choices"][0]["message"]
            content = message.get("content")
        except (KeyError, IndexError):
            raise Exception(f"Unexpected response format: {response}")
        if not content:
            reasoning_details = message.get("reasoning_details", [])
            if reasoning_details:
                reasoning_text = reasoning_details[0].get("text", "") if reasoning_details else ""
                raise Exception(
                    f"Model exhausted max_tokens on reasoning without producing content. "
                    f"Consider increasing max_tokens. Partial reasoning: {reasoning_text[:200]}..."
                )
            raise Exception(f"Model returned null content. Response: {response}")
        return content
